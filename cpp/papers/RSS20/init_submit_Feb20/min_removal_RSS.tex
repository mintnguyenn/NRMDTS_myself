...
\item To solve a simply-connected cell with $K$ edges, the total number of different divisions is bound by %$\Phi(K) \leq 2^K$. 
% <jvm> Tong check pls 
\begin{equation}
\label{equ_Psi}
\Psi(K) = \Phi(K) \leq 2^K
...
% <jvm> Tong pls change `direct solving'' to `direct solution'
\begin{figure}[t]
\centering
...
To show this equivalence, let $\varphi$ be a homeomorphic mapping%~\cite{Simmons1964Introduction} % <jvm> Tong add citation
...
Besides the kinematic, the static force and the manipulability constraints considered in~\cite{Yang2020Cellular_arxiv}, given more complicated constraints and strange thresholds, the emergence of the non simply-connected cells is inevitable. 
...
% <jvm> Tong leave aknw. out for now, there'll be time if accepted 
