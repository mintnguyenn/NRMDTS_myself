...
for i = 1:100
    for j = 1:24
        phi = (i-1)/100*2*pi;
        l = R*(25-j)/24;
        global_ori_ver(i+(j-1)*100, 3) = 0;
        global_ori_ver(i+(j-1)*100, 1) = l*cos(phi);
        global_ori_ver(i+(j-1)*100, 2) = l*sin(phi);
    end
end
global_ori_ver(2401, :) = [0, 0, 0];
...
global_ori_ver(:, 2) = global_ori_ver(:, 2)+0.0;
global_ori_ver(:, 3) = global_ori_ver(:, 3)+0.0;
...
save('UR5_circle_noobstacle.mat');
% load('UR5_ring.mat');
