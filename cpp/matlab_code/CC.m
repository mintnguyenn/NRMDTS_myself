...
cy(1, 1) = collisionCylinder(R, 0.1);
cy(1, 1).Pose = eye(4);
cy(1, 1).Pose(1:3, 4) = cy(1, 1).Pose(1:3, 4) + 0.05*cy(1, 1).Pose(1:3, 3);

cy(2, 1) = collisionCylinder(R, -a2);
T = T01;
T02 = T01*T12;
T(1:3, 1:3) = T02(1:3, 1:3);
T(1:3, 4) = T(1:3, 4) - T(1:3, 1)*(-a2)*0.5;
T(1:3, 1:3) = [T(1:3, 3), T(1:3, 2), -T(1:3, 1)];
cy(2, 1).Pose = T;

cy(3, 1) = collisionCylinder(R, -a3);
T = T01*T12;
T03 = T01*T12*T23;
T(1:3, 1:3) = T03(1:3, 1:3); 
T(1:3, 4) = T(1:3, 4)-T(1:3, 1)*(-a3)*0.5;
T(1:3, 1:3) = [T(1:3, 3), T(1:3, 2), -T(1:3, 1)];
cy(3, 1).Pose = T;

cy(4, 1) = collisionCylinder(r, d4);
T = T01*T12*T23;
T(1:3, 4) = T(1:3, 4)+T(1:3, 3)*d4*0.5;
cy(4, 1).Pose = T;

cy(5, 1) = collisionCylinder(r, d5);
T = T01*T12*T23*T34;
T(1:3, 4) = T(1:3, 4)+T(1:3, 3)*d5*0.5;
cy(5, 1).Pose = T;
