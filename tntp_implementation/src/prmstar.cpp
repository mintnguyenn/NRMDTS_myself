...
void PRMstar::findPath(const RobotState& source, const RobotState& target, std::vector<RobotState>& result){
    ...
    std::vector<boost::graph_traits<
        boost::adjacency_list < boost::listS, boost::vecS, boost::undirectedS, 
            boost::no_property, boost::property < boost::edge_weight_t, double > > >::vertex_descriptor> vPredecessor(boost::num_vertices(g_));
    ...
    boost::graph_traits<
        ...
        = boost::vertex(source_milestone_index, g_);
...
bool PRMstar::getPath(int fromId, int toId, std::vector<boost::graph_traits<
    boost::adjacency_list < boost::listS, boost::vecS, boost::undirectedS, 
